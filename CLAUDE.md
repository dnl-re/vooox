...
Tasks, open issues, and planned features are tracked with **backlog.md**:

```bash
backlog task list --plain   # all tasks
backlog board               # Kanban view
backlog task <id> --plain   # task detail
```

When starting work on a task: `backlog task edit <id> -s "In Progress"`  
...
- Settings window with radio-button mic list and live LevelBar
- System tray, transcription history (50 entries JSONL)
