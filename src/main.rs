...
async fn run_test_pipeline_async(wav_path: &str) -> i32 {
    ...
    if let Err(e) = transcription::whisper_client::wait_for_ready(port, 60).await {
        eprintln!("{e}"); let _ = child.kill(); return 1;
    }
...
fn main() -> glib::ExitCode {
    ...
    let app = Application::builder().application_id("de.vooox.app").build();
...
fn build_ui(app: &Application) {
    ...
    // PTT state: hold_id increments on every Press so the threshold timer can
    // detect if the same hold is still active when it fires.
    ...
    wire_main_event_poll(
        shortcut_rx, tray_rx, app.clone(),
        Rc::clone(&panel), Rc::clone(&config), Rc::clone(&history),
        Rc::clone(&recording), Rc::clone(&recorder),
        Rc::clone(&hold_id), Rc::clone(&ptt_active),
        tray_handle, input_device, port,
    );
    let _hold = app.hold();
    let pid = sidecar.id() as libc::pid_t;
    std::mem::forget(sidecar);
    app.connect_shutdown(move |_| { unsafe { libc::kill(pid, libc::SIGTERM) }; });
}

fn wire_main_event_poll(
    shortcut_rx: crossbeam_channel::Receiver<system::shortcuts::ShortcutEvent>,
    tray_rx: crossbeam_channel::Receiver<AppCommand>,
    app: Application,
    ...
    glib::timeout_add_local(std::time::Duration::from_millis(30), move || {
        while let Ok(ev) = shortcut_rx.try_recv() {
            handle_shortcut_event(ev, &panel, &config, &history, &recording, &recorder, &hold_id, &ptt_active, &tray_handle, &input_device, port);
        }
        while let Ok(cmd) = tray_rx.try_recv() {
            handle_tray_command(cmd, &app, &panel, Rc::clone(&config), Rc::clone(&history), tray_handle.clone(), port);
        }
...
fn handle_shortcut_event(
    ev: system::shortcuts::ShortcutEvent,
    panel: &Rc<DictationPanel>,
    config: &Rc<RefCell<Config>>,
    history: &Rc<RefCell<History>>,
    recording: &Rc<RefCell<bool>>,
    ...
    match ev {
        system::shortcuts::ShortcutEvent::Press => {
            ...
            handle_shortcut_press(this_hold, panel, config, history, recording, recorder, hold_id, ptt_active, tray_handle, input_device, port);
        }
        system::shortcuts::ShortcutEvent::Release => {
            ...
            if ptt_active.get() && *recording.borrow() {
                ...
                stop_recording(Rc::clone(recorder), Rc::clone(recording), Rc::clone(panel), tray_handle.clone(), config.borrow().clone(), Rc::clone(history), port);
            }
            ptt_active.set(false);
        }
    }
...
fn handle_shortcut_press(
    ...
    if *recording.borrow() {
        ...
        start_recording(dev, Rc::clone(recorder), Rc::clone(recording), Rc::clone(panel), tray_handle.clone(), port);
        let cfg_snap = config.borrow().clone();
        if cfg_snap.push_to_talk_enabled {
            let threshold = std::time::Duration::from_millis(cfg_snap.push_to_talk_threshold_ms as u64);
            schedule_ptt_visual_update(this_hold, threshold, Rc::clone(hold_id), Rc::clone(ptt_active), Rc::clone(recording), Rc::clone(panel));
        }
    } else {
...
fn handle_tray_command(
    ...
    match cmd {
        AppCommand::OpenSettings => { ui::settings::SettingsWindow::new(app, config, port).show(); }
        AppCommand::ShowPanel    => { panel.present(); }
        AppCommand::OpenHistory  => { ui::history_window::open(app, history); }
        AppCommand::HidePanel    => { panel.hide(); }
        AppCommand::SetPanelMode(m) => save_panel_mode_and_update_ui(m, config, Rc::clone(panel), tray_handle),
        AppCommand::SetModel(m)     => spawn_model_update_task(m, port, config),
        AppCommand::Quit            => { app.quit(); }
    }
}

fn schedule_ptt_visual_update(
    hold_at_schedule: u64,
    ...
    glib::timeout_add_local_once(threshold, move || {
        if hold_id.get() == hold_at_schedule && *recording.borrow() {
            ptt_active.set(true);
...
fn save_panel_mode_and_update_ui(
    mode: crate::storage::config::PanelMode,
    config: Rc<RefCell<Config>>,
    panel: Rc<DictationPanel>,
    tray_handle: Option<ksni::blocking::Handle<ui::tray::VoooxTray>>,
) {
    {
        ...
        if cfg.panel_mode != mode {
            cfg.panel_mode = mode;
            if let Err(e) = cfg.save() { eprintln!("[config] save: {e}"); }
        }
    }
    panel.apply_mode(mode);
    if let Some(ref h) = tray_handle { ui::tray::set_panel_mode(h, mode); }
}

fn spawn_model_update_task(model: String, port: u16, config: Rc<RefCell<Config>>) {
    {
        let mut cfg = config.borrow_mut();
    ...
    match audio::Recorder::start(dev) {
        Ok(rec) => {
            ...
            *recording.borrow_mut() = true;
            ...
            if let Some(ref h) = tray_handle { ui::tray::set_recording(h, true); }
...
fn stop_recording(
    ...
    *recording.borrow_mut() = false;
    ...
    if let Some(ref h) = tray_handle { ui::tray::set_recording(h, false); }
    if let Some(rec) = recorder.borrow_mut().take() {
        let wav = encode_recorded_audio_to_wav(rec);
        eprintln!("[audio] WAV size: {} bytes", wav.len());
        let client = WhisperClient::new(port);
        let (seg_tx, seg_rx) = bounded::<Result<String, String>>(32);
        spawn_transcription_thread(client, wav, seg_tx);
        spawn_segment_poll(seg_rx, panel, cfg, history);
    }
}

fn encode_recorded_audio_to_wav(rec: audio::Recorder) -> Vec<u8> {
    ...
    eprintln!("[audio] stopped — {} samples, {:.2}s, {}Hz {}ch", samples.len(), duration_s, sample_rate, channels);
    let mono = audio::to_mono(&samples, channels);
    audio::to_wav_bytes(&mono, sample_rate, 1)
}

fn spawn_transcription_thread(client: WhisperClient, wav: Vec<u8>, seg_tx: crossbeam_channel::Sender<Result<String, String>>) {
    std::thread::spawn(move || {
        let rt = tokio::runtime::Runtime::new().unwrap();
...
fn spawn_streaming_timer(
    ...
    glib::timeout_add_local(std::time::Duration::from_millis(200), move || {
        if !*recording.borrow() { return glib::ControlFlow::Break; }
        if let Some(text) = collect_available_transcript_result(&stream_rx) {
            if *recording.borrow() { panel.set_transcript(&text); }
        }
        maybe_start_streaming_transcription(&recorder, &stream_rx, &stream_last_len, port);
        glib::ControlFlow::Continue
    });
}

fn collect_available_transcript_result(stream_rx: &Rc<RefCell<Option<StreamRx>>>) -> Option<String> {
...
fn maybe_start_streaming_transcription(
    recorder: &Rc<RefCell<Option<audio::Recorder>>>,
    stream_rx: &Rc<RefCell<Option<StreamRx>>>,
    stream_last_len: &Rc<RefCell<usize>>,
    port: u16,
) {
    if stream_rx.borrow().is_some() { return; }
    let Some(captured) = peek_audio_if_enough_accumulated(recorder, stream_last_len) else { return; };
    ...
    *stream_rx.borrow_mut() = Some(rx);
    dispatch_streaming_transcription(wav, port, tx);
}

fn peek_audio_if_enough_accumulated(
    recorder: &Rc<RefCell<Option<audio::Recorder>>>,
    stream_last_len: &Rc<RefCell<usize>>,
) -> Option<audio::CapturedAudio> {
    let rec_opt = recorder.borrow();
    let rec = rec_opt.as_ref()?;
    let count = rec.sample_count();
    let min_new = rec.sample_rate as usize * rec.channels as usize * 3;
    if count >= *stream_last_len.borrow() + min_new { Some(rec.peek_samples()) } else { None }
}

fn dispatch_streaming_transcription(wav: Vec<u8>, port: u16, tx: crossbeam_channel::Sender<Option<String>>) {
    std::thread::spawn(move || {
        let rt = tokio::runtime::Runtime::new().unwrap();
        let client = WhisperClient::new(port);
...
fn spawn_segment_poll(
    ...
    glib::timeout_add_local(std::time::Duration::from_millis(50), move || {
        poll_segment_rx_until_pause(&seg_rx, &panel, &full_text, &first_seg, &cfg, &history)
    });
}

fn poll_segment_rx_until_pause(
    seg_rx: &crossbeam_channel::Receiver<Result<String, String>>,
    panel: &Rc<DictationPanel>,
    full_text: &Rc<RefCell<String>>,
    first_seg: &Rc<RefCell<bool>>,
    cfg: &Config,
    history: &Rc<RefCell<History>>,
) -> glib::ControlFlow {
    loop {
        match seg_rx.try_recv() {
            Ok(Ok(seg)) => accumulate_segment(panel, full_text, first_seg, seg),
            Ok(Err(e)) => {
                eprintln!("[whisper] {e}");
